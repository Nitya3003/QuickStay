...
const HotelReg = () => {
  ...
  const onSubmitHandler = async (event) => {
    try {
      event.preventDefault();            // Stop webpage from refreshing on form submission
  ...
  return (
    ...
    <input id='name' onChange = {(e) => setName(e.target.value)} value={name} type="text" placeholder="Type here" className="border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light"
        required
    />
    </div>
